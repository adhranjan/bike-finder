        ...
        <input type="radio" name="choice" <?php if ($value == 'price') { ?>checked='checked'<?php } ?> onChange="autoSubmit();" value="price"> Price
     	 ...
     	 <input type="radio" name="choice" <?php if ($value == 'stockRem') { ?> checked='checked'<?php } ?> onChange="autoSubmit();" value="stockRem"> Number of Availabe bikes
								...
								<div class="text">
							 	...
							 	<?php
							 	if($value==date){
							 	$query="SELECT * FROM bike b  inner join user u on u.userID= b.userID WHERE (ItemRemaining>0 AND bikeVisibility=1) Order By bikeID DESC  LIMIT 6" ; //query
								...
								while ( $row = mysqli_fetch_array($result, MYSQL_BOTH)) {
							 	...
							 	$query="SELECT * FROM bike b  inner join user u on u.userID= b.userID WHERE (ItemRemaining>0 AND bikeVisibility=1 ) ORDER BY bikePrice ASC LIMIT 6" ; //query
									$result=$db->query($query);
								while ( $row = mysqli_fetch_array($result, MYSQL_BOTH)) {
									$bikeID=$row[0];
									$genericName=$row[1];		
								...
								if($value==stockRem){
							 	$query="SELECT * FROM bike b  inner join user u on u.userID= b.userID WHERE (ItemRemaining>0 AND  ItemRemaining<=3 AND bikeVisibility=1) ORDER BY ItemRemaining ASC LIMIT 6" ; //query
									$result=$db->query($query);
								while ( $row = mysqli_fetch_array($result, MYSQL_BOTH)) {
									$bikeID=$row[0];
									$genericName=$row[1];		
							 	...
							 	<div class='grid_1_of_3 images_1_of_3'>
									...
									if($rowCount==0)
									{
							 	$query="SELECT * FROM bike b  inner join user u on u.userID= b.userID WHERE (ItemRemaining>0 AND  ItemRemaining<=10 AND bikeVisibility=1) ORDER BY ItemRemaining ASC LIMIT 6" ; //query
									$result=$db->query($query);
								while ( $row = mysqli_fetch_array($result, MYSQL_BOTH)) {
									$bikeID=$row[0];
									$genericName=$row[1];		
